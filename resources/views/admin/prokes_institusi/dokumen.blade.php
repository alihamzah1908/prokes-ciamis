...
<div class="card-header card-header-primary">
    <h4 class="card-title">Upload Dokumen Prokes Institusi</h4>
    <!-- <p class="card-category">Complete your profile</p> -->
</div>
<ol class="breadcrumb mb-4 mt-4">
    <li class="breadcrumb-item"><a href="{{ route('institusi.index') }}">Prokes Institusi </a></li>
    <li class="breadcrumb-item active">Dokumen Institusi</li>
</ol>
@php 
if(Auth::user()->role == 'Staff'){
    ...
    $val2 = \App\Models\ProkesInstitusi::find(request()->institusi_id);
    $kode_desa = $val2->desa_id ?? '';
    $kode = Auth::user()->kode_kecamatan;
    $kode_kecamatan = Auth::user()->kode_kecamatan;
}else{
    $kode = $kode = Auth::user()->id;
    $val2 = \App\Models\ProkesInstitusi::find(request()->institusi_id);
    $kode_kecamatan = $val2->kecamatan_id;
    $kode_desa = $val2->desa_id ?? '';
}
@endphp
...
<div class="tab-content">
  <div class="tab-pane active" id="data">
    <form action="{{ route('upload.dokumen_institusi') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="row mt-4">
        ...
        @endif
            ...
            <div class="row mb-3">
                ...
                <div class="col-md-6">
                    @php
                    $xpld = explode(' - ', $val2->lokasi_pantau);
  ...
  <div class="tab-pane fade" id="dokumentasi"> 
    <div class="row ml-1">
        @php 
        $image = \App\Models\DokumenInstitusi::where('institusi_id', request()->institusi_id)->get();
        @endphp
        @foreach($image as $val)
        <a target="_blank" href="{{ asset('dokumen_institusi') }}/{{ $val->image }}">  
            <img src="{{ asset('dokumen_institusi') }}/{{ $val->image }}" width="100" height="100" style="margin-right: 20px;margin-top: 10px;"/>
        </a>
        @endforeach
    </div>
  </div>
...
<script>
    $(document).ready(function(){
        ...
        $('body').on('click','.tambah-kolom', function(){
            ...
            body += '<input type="file" name="image['+ id +']" class="form-control image" required placeholder="isi tanggal pantau" required>';
            body += '</div>';
            body += '</div>';
            body += '</div>';
            $('.kolom').append(body)
